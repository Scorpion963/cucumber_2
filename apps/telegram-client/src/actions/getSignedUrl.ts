...
export async function getSignedPutUrl(
  ...
  if (!user) {
    return { error: {code: ReasonPhrases.UNAUTHORIZED, message: "You must be logged in to change the image"}, data: null };
  }

  if (!ALLOWED_CONTENT_TYPES.includes(contentType))
    return { error: {code: ReasonPhrases.UNPROCESSABLE_ENTITY, message: "Invalid data"}, data: null };
